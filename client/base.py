...
from .request import (
    ...
    ClanBattlePeriodRankingRequest,
    ArenaInfoRequest,
    ArenaRankingRequest,
...
from .response import (
    ...
    ClanBattlePeriodRankingResponse,
    ArenaInfoResponse,
...
class BaseClient:
    ...
    def unpack(self, data: bytes) -> Tuple[dict, bytes]:  # 与原版不同，优先改
        data = b64decode(data.decode("utf8"))
        if len(data) <= 32:
            # 响应不是标准 AES 加密格式（可能接口不被支持/参数有误/被网关拦截），
            # 记录原始内容便于排查，而不是抛出难以理解的 AES key length 错误
            logger.error(f"api 响应非标准加密格式，原始内容: {data[:500]!r}")
            raise ValueError(f"api response not standard: {data[:200]!r}")
        aes = AES.new(data[-32:], AES.MODE_CBC, self._getiv())
    ...
    async def clan_battle_period_ranking(
        self, clan_id: int, clan_battle_id: int, page: int = 0, is_my_clan: int = 0
    ) -> ClanBattlePeriodRankingResponse:
        """会战期间全服公会排名（每页 10 条，page 从 0 开始，跳页直接取目标档位）"""
        response = await self.callapi(
            ClanBattlePeriodRankingRequest(
                clan_id=clan_id,
                clan_battle_id=clan_battle_id,
                page=page,
                is_my_clan=is_my_clan,
            )
        )
        parsed = ClanBattlePeriodRankingResponse.parse_obj(response)
        if not parsed.ranking_list:
            logger.warning(f"period_ranking 空数据，原始响应: {str(response)[:800]}")
        return parsed
